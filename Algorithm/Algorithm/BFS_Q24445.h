...
private:
	...
	uint _N, _M, _R;
	uint _cnt;
	std::vector<std::vector<uint>> _nodes;
	std::vector<uint> _visited;
private:
	virtual void Input()
	{
		Q_INPUT_BEGIN();
		...
		_cnt = 0;
		_nodes.resize(_N + 1);
		_visited.resize(_N + 1, 0);
		uint u, v;
		for (uint i = 0; i < _M; ++i)
		{
			cin >> u >> v;
			_nodes[u].emplace_back(v);
			_nodes[v].emplace_back(u);
		}
