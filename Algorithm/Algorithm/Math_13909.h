...
private:
	...
	std::vector<uint> _squareArr;
	uint _n;

	inline static uint MAX_NUM = 2'100'000'000;
private:
	void SetSquareArray()
	{
		if (_squareArr.empty())
			for (uint i = 1; i * i < MAX_NUM + 1; ++i)
				_squareArr.push_back(i * i);
	}
	virtual void Input()
	{
		Q_INPUT_BEGIN();
