...
private:
	...
	{
		Q_INPUT_BEGIN();
		board.resize(5, std::vector<char>(5));
		visited.resize(5, std::vector<bool>(5, false));
	...
	void dfs(int x, int y, std::string currentWord) {
		// 현재 단어가 사전에 있으면 추가
		if (dictionary.count(currentWord) && currentWord.length() >= 3) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (isValid(nx, ny)) {
				visited[nx][ny] = true;
				currentWord += board[nx][ny];

				dfs(nx, ny, currentWord);
	...
	virtual void Solution()
	{
		...
		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				visited[i][j] = true;
				std::string startWord(1, board[i][j]);
				dfs(i, j, startWord);
				visited[i][j] = false;
			}
		}

		std::cout << foundWords.size() << std::endl;
