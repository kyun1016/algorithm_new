...
* Q10810
--------------------*/
class Q10810 : public QBase
{
public:
	Q10810() = default;
	virtual ~Q10810() = default;

private:
	virtual void Input(const int& testCase)
	{
#if defined(DEBUG) || defined(_DEBUG)
		std::ifstream cin = QHelper::LoadTestInput("./TestData/Q10810/", testCase);
#else
		using namespace std;
#endif
		cin >> _N >> _M;
		_arr.resize(_M, std::vector<uint16_t>(3, 0));
		for (auto& a : _arr)
			cin >> a[0] >> a[1] >> a[2];
	}
	...
	{
#if defined(DEBUG) || defined(_DEBUG)
		std::ofstream cout = QHelper::PrintTestAnswer("./TestData/Q10810/", testCase);
#else
		using namespace std;
#endif
		std::vector<uint16_t> ans(_N, 0);

		for (const auto& a : _arr)
			for (uint16_t i = a[0] - 1; i < a[1]; ++i)
				ans[i] = a[2];
		...
		cout << std::endl;
		cout.close();
		QHelper::Score("./TestData/Q10810/", testCase);
#endif
	}
	virtual void Delete()
	{
...
{
#if defined(DEBUG) || defined(_DEBUG) 
	// QHelper::SaveTest("./TestData/Q10810/", 1);
	QHelper::Init();
#endif

	std::unique_ptr<QBase> q = std::make_unique<Q10810>();
