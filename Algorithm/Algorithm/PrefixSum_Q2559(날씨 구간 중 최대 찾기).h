...
* Q2559
--------------------*/
class Q2559 final : public QBase
{
public:
	Q2559() = default;
	virtual ~Q2559() = default;
...
private:
	...
	virtual void Solution(const int& testCase) override
	{
		...
		* Solution
		--------------------*/
		{
			std::vector<int32_t> sumArr;
			sumArr.resize(_N + 1);
			for (uint32_t i = 0; i < _N; ++i)
				sumArr[i + 1] = sumArr[i] + _arr[i];

			int32_t ret = -999999;
			for (uint32_t i = _K; i <= _N; ++i)
				ret = std::max(ret, sumArr[i] - sumArr[i - _K]);
...
private:
	std::string _dir = "./TestData/Q2559/";
private:
...
#if defined(DEBUG) || defined(_DEBUG)
	int TestCase = 2;
	// QHelper::SaveTest("./TestData/Q2559/", TestCase);
	std::unique_ptr<QBase> q = std::make_unique<Q2559>();
