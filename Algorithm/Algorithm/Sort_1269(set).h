...
private:
	...
	virtual void Solution()
	{
		...
		std::set<int> _setAsubB(_setA);
		std::set<int> _setBsubA(_setB);
		for (const auto& num : _setB)
			_setAsubB.erase(num);
		for (const auto& num : _setA)
			_setBsubA.erase(num);

		cout << _setAsubB.size() + _setBsubA.size() << '\n';
